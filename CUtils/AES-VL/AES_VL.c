...
void aes_cypher_128_cbc_enc(char* filename,char* outfilename)
{
    //申请一个标准的4kbyte的缓冲区，读取数据
    uint8_t buf[40960] = {0};
    uint8_t key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                      0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
    uint8_t iv[] = {1,2,3,4,2,2,3,4,3,2,3,4,4,2,3,4};
	...
	while ((readlength = fread(buf, sizeof(uint8_t), 40960, file)) > 0)
	{
        printf("read %lu",readlength);
		aes_encrypt_cbc(AES_CYPHER_128, buf, readlength, key,iv);
		fwrite(buf, sizeof(uint8_t), readlength, outfile);
	}
    printf("read %lu",readlength);
	fclose(outfile);
...
void aes_cypher_128_enc(char* filename,char* outfilename)
{
    //申请一个标准的4kbyte的缓冲区，读取数据
    uint8_t buf[4096] = {0};
    uint8_t key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
	...
	while ((readlength = fread(buf, sizeof(uint8_t), 4096, file)) > 0)
	{
		aes_encrypt(AES_CYPHER_128, buf, readlength, key);
		fwrite(buf, sizeof(uint8_t), readlength, outfile);
	}
	fclose(outfile);
...
void aes_cypher_128_dec(char * filename,char * outfilename)
{
    //申请一个标准的4kbyte的缓冲区，读取数据
    uint8_t buf[4096] = {0};
    uint8_t key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
	...
	while ((readlength = fread(buf, sizeof(uint8_t), 4096, file)) > 0)
	{
		aes_decrypt(AES_CYPHER_128, buf, readlength, key);
		fwrite(buf, sizeof(uint8_t), readlength, outfile);
	}
	fclose(outfile);
    ...
    */
	if (argc >1)
	{
        if(strcmp(argv[1],"-enc")==0 && argc==4)
		{
            ...
            printf("%s",filename);
			aes_cypher_128_cbc_enc(filename,outfilename);
